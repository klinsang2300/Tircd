...
export const getYears = (initialYear = new Date().getFullYear()) => {
    const currentYearGroupStart = Math.floor(initialYear / 10) * 10
    const selectedYear = initialYear;
    const years = [];
    for (let i = 0; i < 16; i++) { // แสดงประมาณ 12 ปี
        const year = currentYearGroupStart + 543 + i;

        years.push({ "id": i, "Year": year, "act": selectedYear === currentYearGroupStart + i });
    }
    return years
